{{-- resources/views/public/layouts/grid.blade.php --}}

<div class="gallery-grid {{ $rounded ? 'gallery-rounded' : '' }}" data-ratio="{{ $ratio }}">
    @foreach ($images as $image)
        @php
            // Escolhe a URL conforme o tamanho solicitado no shortcode
            $url = match ($size) {
                'thumb'    => $image->thumbnail_url ?? $image->url,
                'original' => $image->original_url ?? $image->url,
                default    => $image->url, // 'medium' ou 'large' usam a URL padrão (já redimensionada)
            };
        @endphp
